...
EASTERN_ARABIC_NUMERALS = [u'۰', u'۱', u'۲',
                           u'۳', u'٤', u'۵', u'٦', u'۷', u'۸', u'۹']

eastern_to_western_numerals = {}
for i in range(len(EASTERN_ARABIC_NUMERALS)):
    eastern_to_western_numerals[EASTERN_ARABIC_NUMERALS[i]
                                ] = WESTERN_ARABIC_NUMERALS[i]
...
arabic_punctuations = COMMA + SEMICOLON + QUESTION + PERCENT + DECIMAL + \
    THOUSANDS + STAR + FULL_STOP + MULITIPLICATION_SIGN + DIVISION_SIGN
...
model = keras.models.load_model("static/offensiveDetector.h5")
...
def prepare_text():
    ...
    r = str(text)
    text_after_preprocessing = normalize_arabic_text(r)
    msg = []
    msg.append(text_after_preprocessing)

    with open("static/features.txt", "rb") as f:
        loaded_names = pickle.load(f, encoding="utf8")
    count_vector = CountVectorizer()
    count_vector.fit(loaded_names)
    vector = count_vector.transform(msg)
    prediction = model.predict(vector)
    if float(prediction)*100 > 0.5:
        return "offensive"
    else:
